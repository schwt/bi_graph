...
bool BiGraph::TrainMultiThreads() {
    ...
    // 线程结果池
    vector<hash_map<int, vector<SimInvert> > > vec_threads_result;
    vec_threads_result.resize(num_threads_, NULL);
    ...
    // 配置线程参数
    struct ThreadArgs thArgs(num_threads_, time_decay_type_, top_reserve_, num_item_right_, lambda_, rho_, tau_, score_threshold_, 
            vec_item_id_left_, vec_item_id_right_, vec_item_right_norm_, vec_matrix_idx_item_, 
            vec_matrix_idx_user_, vec_matrix_ivt_item, vec_matrix_ivt_user);
    ...
    for (int id_thread = 0; id_thread < num_threads_; id_thread++) {
        res = !pthread_create(&tid[id_thread], NULL, _trainEachThread, (void*)&vec_args[id_thread]);
        if (!res) {
            printf("Create threads failed!\n");
            return res;
        }
        else printf("Create threads: tid[%d]=%d\n", id_thread, (int)tid[id_thread]);
    }
    ...
    timer.EndTiming();
    logger.info("use time: %ld\n", timer.UsedSeconds());

    // 检查各线程返回值
    for (int id_thread = 0; id_thread < num_threads_; id_thread++) {
        if (!(bool)thread_ret[id_thread]) {
            printf("Thread [%d] %d return false!\n", id_thread, (int)tid[id_thread]);
            return false;
        } else {
    ...
    // 搜集结果
    ret = CombineThreadResults(vec_threads_result);
    if (!ret) {
...
void *_trainEachThread(void *args) {
    ...
    hash_map<int, vector<SimInvert> > hm_result;
    int id_thread = thArgs.thread_id;
    ...
    for (size_t pid = 0; pid < vec_matrix_idx_item.size(); pid ++) {
        ...
        for (int i = 0; i < collector_id; i++) {
            int id = vec_collector_id[i];
            vec_ivt_score[i].id = thArgs.vec_item_id_right[id];
            vec_ivt_score[i].score = vec_collector_score[id] / thArgs.item_right_norm[id];
            sum += vec_ivt_score[i].score;
            vec_collector_score[id] = -1;
        }
        int len =  Min(top_reserve_, collector_id);
        ...
        th_result.insert(make_pair(thArgs.vec_item_id_left[pid], vec_ivt_score));
    }
